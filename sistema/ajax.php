...
if (!empty($_POST)) {
  ...
  if ($_POST['action'] == 'anularFactura') {
    if (!empty($_POST['noFactura'])) {
      ...
      $queryr_anular = mysqli_query($conection,"CALL anular_factura($noFactura)");
      mysqli_close($conection);
      $result = mysqli_num_rows($queryr_anular);

      if ($result >0) {
        $data = mysqli_fetch_assoc($queryr_anular);
        echo json_encode($data,JSON_UNESCAPED_UNICODE);
        exit;
      }
    }
  ...
  // Eliminar un producto agregado (devulucion)
  if ($_POST['action'] == 'devProductoDetalle')
  {
    if (empty($_POST['id_detalle']) || empty($_POST['nofactura'])) {
      echo "error";
    }else{
      ...
      $query_detalle_temp = mysqli_query($conection,"CALL dev_detalle_fac($id_detalle,$nofactura)");
      $result = mysqli_num_rows($query_detalle_temp);
      ...
      if ($result > 0){
        ...
        while ($data = mysqli_fetch_assoc($query_detalle_temp)){
          ...
          $detalleTabla .= '<tr>
                            ...
                            <td class="textright">'.$precioTotal.'</td>
                            <td class="">
                              <a href="#" class="link_delete" onclick="event.preventDefault(); dev_product_detalle('.$data['correlativo'].','.$data['nofactura'].');"><i class="far fa-trash-alt"></i></a>
                            </td>
                          </tr>';
        }
        $impuesto = round($sub_total * ($iva / 100), 2);
